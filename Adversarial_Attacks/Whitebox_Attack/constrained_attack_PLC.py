...
def sort_temp_and_drop(row_index, temp):
    ...
    for col in temp.columns:
        if temp.loc[row_index, col] < theta:
            break
        i = i + 1
    temp = temp.drop(columns=temp.columns[i:])
    return(temp.copy())
...
def choose_column(row_index, prev_col_name, changed_variables, len_plc_elements, temp):
    """
    ...
    """
    intersection =  list(set(temp.columns) & set(changed_variables[row_index]))
    intersection_sorted = [x for x in temp.columns if x in set(intersection)]
    if not(intersection_sorted) or (len(intersection_sorted)<=2):
        return changed_variables[row_index][np.random.randint(0, len(changed_variables[row_index]))]
    if (prev_col_name == intersection_sorted[0]):
        chosen = intersection_sorted[np.random.randint(1, len(intersection_sorted))]
        return chosen 
    return intersection_sorted[0]
...
def change_vector_label(row_index, att_data, solutions_found, changed_variables, variables):
    ...
    optimized = False
    changed_variables[row_index] = variables[PLC]
    while changes < budget and (changes - last_optimization) < patience and not(found_solution):
        col_name = choose_column(row_index, prev_col_name,
                                 changed_variables, len(variables[PLC]), temp)
...
parser.add_argument('-a', '--att', nargs='+', type = int)
parser.add_argument('-p', '--plc', nargs='+', type = str)
...
for att_number in attack_ids:
    variables = {}
    f = open("./constraints/"+dataset+"/constraint_PLC.txt"
             , 'r').read()
    variables = eval(f)
    for PLC in args.plc:
        debug = False
        ...
        print('CONSTRAINTS:'+str(variables[PLC]))
        ...
        new_tuples.to_csv('./results/'+dataset+'/max_constraints_fixed/whitebox_attack_' +
                          str(att_number)+'_from_test_dataset'+PLC+'.csv')
        ...
        with open('./results/'+dataset+'/max_constraints_fixed/time_spent_new_sequential_v2.0_fixed_budget.txt', 'a') as f:
            f.write('______attack: '+str(att_number)+'constraint: '+PLC+'______\n')
