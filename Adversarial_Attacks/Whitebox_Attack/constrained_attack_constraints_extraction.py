...
def choose_column(row_index, temp, prev_col_name, num_changes_without_optimizations, changed_variables, max_concealable_variables):
    try:
        changed_variables[row_index]
    except:
        return temp.columns[0]
    # print(changed_variables)
    if len(changed_variables[row_index]) < max_concealable_variables:
        if num_changes_without_optimizations >= 2:
            return temp.columns[np.random.randint(
                0, temp.columns.size)]
        else:
            if temp.columns[0] == prev_col_name:
                return temp.columns[1]
            else:
                return temp.columns[0]
    else:
...
def change_vector_label(row_index, att_data, solutions_found, changed_variables):
    """
...
for att_number in attack_ids:
    ...
    for max_concealable_variables in [2, 3, 4, 5, 6, 7, 8, 9, 10, 15, 20]:
        ...
        for row_index, row in X.iterrows():
            ...
            if Yhat[row_index]:
                ...
                modified_row, solutions_found = change_vector_label(
                    row_index, prov, solutions_found, changed_variables)
        ...
        for key in changed_variables.keys():
            binary_row = pd.DataFrame(index=[key], columns=xset, data=0)
            for elem in changed_variables[key]:
                binary_row.loc[key, elem] = 1
        ...
        new_tuples.to_csv('./results/'+dataset+'/max_constraints_search/new_improved_whitebox_attack_' +
                          str(att_number)+'_from_test_dataset_max_'+str(max_concealable_variables)+'.csv')
        print('mean spent time: ' + str(sum_spent/changed_rows))
        with open('./results/'+dataset+'/max_constraints_search/time_spent_new_sequential_v2.0.txt', 'a') as f:
        ...
        print(variables)
    with open("./constraints/"+dataset+"/constraint_variables_attack_"+str(att_number)+".txt", 'w') as f:
