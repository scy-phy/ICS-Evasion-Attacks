    ...
    def build_generator(self):
        ...
        for dim in self.generator_layers[1:]:
            x = Dense(dim, activation='sigmoid',
                      kernel_initializer=keras.initializers.glorot_normal(seed=12345))(x)
    ...
    def fix_sample(self, gen_examples, dataset):
        ...
        if dataset == 'WADI':
            list_pump_status = list(gen_examples.filter(
                regex='P_[0-9][0-9][0-9]_STATUS').columns)
            list_MV_status = list(gen_examples.filter(
                regex='MV_[0-9][0-9][0-9]_STATUS').columns)
            for j, _ in gen_examples.iterrows():
                for i in list_pump_status:
                    if gen_examples.at[j, i] > 1.5:
                        gen_examples.at[j, i] = 2
                    else:
                        gen_examples.at[j, i] = 1
                for i in list_MV_status:
                    if gen_examples.at[j, i] > 1.5:
                        gen_examples.at[j, i] = 2
                    else:
                        if gen_examples.at[j, i] > 0.5:
